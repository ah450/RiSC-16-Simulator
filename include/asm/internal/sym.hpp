...
public:
    /**
     * @brief Creates an undefined label i.e forward declaration. global
     */
    Sym( const std::string &name );
    Sym( const std::string &name, const bfs::path &file );
    ...
    bool operator<( const Sym &other ) const {
        return ( file_.generic_string() +  name_ ) < ( other.file_.generic_string() +
                other.name_ );
    }
