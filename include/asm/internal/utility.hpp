...
namespace ass {
namespace internal {
inline std::tuple<std::uint8_t, std::uint8_t, std::uint8_t> getRegsFromArithm(
    boost::smatch &m ) {
...
inline std::uint16_t unpackRegs(
    std::tuple<std::uint8_t, std::uint8_t, std::uint8_t> &regs ) {
    return ( std::get<0>( regs ) << 6 ) | ( std::get<1>( regs ) << 3 ) |
           std::get<2>( regs );
}
...
inline void fillInstruction( Instruction &i, addr_t value ) {
    switch ( i.type ) {
        ...
        case InstType::BEQ:
            i.data |= ( ( 0x7F & value ) - ( i.pc + 2 ) ) & 0x7F;
            break;

        case InstType::JMP :
        case InstType::JALI:
            i.data |= ( ( 0x03FF & value ) - ( i.pc + 2 ) ) & 0x03FF;
            break;
        ...
        default:
            throw std::runtime_error( "PROGRAMMER OOPSY !!!!! :< " );
    }
}
 ...
 */
inline void resolveLabel( const std::string &name, AssemblingStatus &state,
                          FileState &tu,
    ...
    for ( auto &ref : tu.importedSyms ) {
        if ( ref.ref->name() == name ) {
            ...
            if ( ref.ref->defined() ) {
                ...
                state.deps().insert( std::pair<SymReference, pc_t>( ref, i.pc / 2 ) );
            }
        }
    }
        ...
        for ( auto &ref : tu.exportedSyms ) {
            if ( ref.ref->name() == name ) {
                ...
                if ( ref.ref->defined() ) {
                    ...
                    tu.localDeps.insert( std::pair<SymReference, pc_t>( ref, i.pc / 2 ) );
                }
            }
        }
    }
    ...
    if ( !filled ) {
        ...
        auto sr = SymReference( std::make_shared<Sym>( name, tu.name ) );
        tu.localSyms.insert( sr );
        tu.localDeps.insert( std::pair<SymReference, pc_t>( sr, i.pc / 2 ) );
    }
}
